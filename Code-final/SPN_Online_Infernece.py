...
from core.model import SPN
from configs.spn_config import Config
#import mkl
...
def execute(config, training_data, training_label, training_index, training_length,
    ...
    model = SPN(hidden_size = config.hidden_size)
...
raw_data_config = DataConfig(dataset_name = use_dataset_name, segmenter = "christov")
...
if(use_dataset_name == "PTBXL"):
    ...
    data, Y, labels, classes = uio.load_formmated_raw_data(raw_input_path, "superdiagnostic", raw_tmp_path)
...
config = Config(model_name="SPN_V1",
                beta=0.00001,
                epoch_size=1,
